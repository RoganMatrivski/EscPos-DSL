...
package id.my.rgmtrv.escposdsl
...
private object Native {
    init {
        val androidLibrary = "escpos_dsl"
        val desktopPreferredLibrary = "escpos_dsl_jni"
        val desktopFallbackLibrary = "escpos_dsl"
    ...
    @JvmStatic external fun boltffi_function_escpos_dsl_compile_dsl(dsl: java.nio.ByteBuffer, __boltffi_dsl_len: Int, max_chars_per_line: Byte): ByteArray?
    @JvmStatic external fun boltffi_function_escpos_dsl_compile_dsl_raw(dsl: java.nio.ByteBuffer, __boltffi_dsl_len: Int, max_chars_per_line: Byte): ByteArray?
}


data class ReceiptBytes(
    val bytes: ByteArray
) {
    ...
    companion object {
        internal fun fromReader(reader: WireReader): ReceiptBytes {
            return ReceiptBytes(
                reader.readBytes()
            )
        }

        internal fun fromByteArray(bytes: ByteArray): ReceiptBytes {
...
fun compileDsl(dsl: String, maxCharsPerLine: UByte): ReceiptBytes {
    val __boltffi_dsl_wire = WireWriterPool.acquire(4 + Utf8Codec.maxBytes(dsl))
    val __boltffi_dsl_writer = __boltffi_dsl_wire.writer
    __boltffi_dsl_writer.writeString(dsl)
    try {
        val __boltffi_result = try { Native.boltffi_function_escpos_dsl_compile_dsl(__boltffi_dsl_wire.directBuffer(), __boltffi_dsl_wire.size(), maxCharsPerLine.toByte()) } catch (__boltffi_error: BoltFfiErrorBufferException) { run { val __boltffi_error_reader = WireReader(__boltffi_error.bytes); throw FfiException(__boltffi_error_reader.readString()) } } ?: throw IllegalStateException("null buffer returned")
        val __boltffi_reader = WireReader(__boltffi_result)
        return ReceiptBytes.fromReader(__boltffi_reader)
    } finally {
        __boltffi_dsl_wire.close()
    }
}

fun compileDslRaw(dsl: String, maxCharsPerLine: UByte): ByteArray {
    val __boltffi_dsl_wire = WireWriterPool.acquire(4 + Utf8Codec.maxBytes(dsl))
    val __boltffi_dsl_writer = __boltffi_dsl_wire.writer
    __boltffi_dsl_writer.writeString(dsl)
    try {
        val __boltffi_result = try { Native.boltffi_function_escpos_dsl_compile_dsl_raw(__boltffi_dsl_wire.directBuffer(), __boltffi_dsl_wire.size(), maxCharsPerLine.toByte()) } catch (__boltffi_error: BoltFfiErrorBufferException) { run { val __boltffi_error_reader = WireReader(__boltffi_error.bytes); throw FfiException(__boltffi_error_reader.readString()) } } ?: throw IllegalStateException("null buffer returned")
        val __boltffi_reader = WireReader(__boltffi_result)
